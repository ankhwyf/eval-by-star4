function submitDetail(param) {
    var scores = document.getElementsByClassName(param + '-score');
    var proofs = document.getElementsByClassName(param + '-proof');

    var scoreInput = document.getElementsByClassName(param + '-score-input');
    var proofInput = document.getElementsByClassName(param + '-proof-input');
...
function addLoadLine(a) {
    ...
    for (var i = 0; i < a; i++) {
        ...
        tpl += "</div>";
        tpl += "<input class=\"effort-input\" name=\"load-" + i + "\" type=\"hidden\" />";
        tpl += "</td>";
    }
...
function addLoadClick(a,b){
    $('.load-table').append(addLoadLine(a,b));
...
$(document).ready(function(){delLoad();});
